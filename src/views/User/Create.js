...
const UserCreate = (props) => {
	...
	const isCreateUserPossible = (username, email, password1, password2) => {
		const usernameValid = username.length > 2 && username.indexOf("@") !== -1;
	...
	const onChangeUsername = (event) => {
		...
		setCreateUserPossible(
			isCreateUserPossible(event.target.value, email, password1, password2),
		);
	};
	...
	const onChangeEmail = (event) => {
		...
		setCreateUserPossible(
			isCreateUserPossible(username, event.target.value, password1, password2),
		);
	};
					...
					<RegularCard
								...
								<Grid container>
									<GridItem xs={12} sm={12} md={6}>
										<CustomInput
											...
											id="username"
											helperText={
												errorsDict.hasOwnProperty("username")
											...
											error={errorsDict.hasOwnProperty("username")}
										/>
									</GridItem>
									<GridItem xs={12} sm={12} md={6}>
										<CustomInput
											...
											id="email"
											helperText={
												errorsDict.hasOwnProperty("email")
													? t(errorsDict["email"])
													: ""
											}
											formControlProps={{
											...
											error={errorsDict.hasOwnProperty("email")}
										/>
									</GridItem>
									<GridItem xs={12} sm={12} md={6}>
										<CustomInput
											...
											id="password1"
											helperText={
												errorsDict.hasOwnProperty("password")
											...
											error={errorsDict.hasOwnProperty("password")}
										/>
									</GridItem>
									<GridItem xs={12} sm={12} md={6}>
										<CustomInput
											...
											id="password2"
											helperText={
												errorsDict.hasOwnProperty("password2")
											...
											error={errorsDict.hasOwnProperty("password2")}
										/>
									</GridItem>
								</Grid>
							</div>
						...
						footer={
							<div>
								...
								{errorsDict.hasOwnProperty("non_field_errors") ? (
									<SnackbarContent
										message={t(errorsDict["non_field_errors"])}
										color="danger"
									/>
								) : (
									""
