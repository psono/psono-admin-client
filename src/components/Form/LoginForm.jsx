...
const LoginForm = (props) => {
			...
			(res) => {
				if (res.data.non_field_errors === undefined) {
					...
					if (serverCheck.info.multifactor_enabled === false) {
						helper.removeFromArray(requiredMultifactors, "ivalt_2fa");
					} else {
					...
					errorsResponses[res.data.non_field_errors[0]] !== undefined &&
					...
					.then(
						(successful) => {
							if (serverCheck.info.multifactor_enabled === false) {
								...
								helper.removeFromArray(multifactors, "webauthn_2fa");
	...
	const has_ldap_auth = (server_check) => {
		return (
			...
			server_check["info"]["authentication_methods"].indexOf("LDAP") !== -1
		);
	};
			...
			.then(
				(result) => {
					...
					if (result.status !== "matched") {
						...
						props.get_saml_redirect_url(providerId).then((result) => {
			...
			.then(
				(result) => {
					...
					if (result.status !== "matched") {
						...
						props.get_oidc_redirect_url(providerId).then((result) => {
	...
	const approveHost = () => {
		...
		if (loginType === "SAML") {
			...
			if (plainPasswordWhitelistedServerUrls.includes(serverCheck.server_url)) {
				let passwordCopy = password;
				setPassword("");
			...
			props.login(passwordCopy, serverCheck).then(handleLogin, (result) => {
				setLoginLoading(false);
				if (result.hasOwnProperty("non_field_errors")) {
					let errors = result.non_field_errors;
					setErrors(errors);
	...
	const onNewConfigLoaded = (adminClientConfig) => {
		...
		const plainPasswordWhitelistedServerUrls = adminClientConfig.backend_servers
	...
	const errorGridItem = (
		<GridItem xs={8} sm={8} md={8} style={{ marginTop: "20px" }}>
			{errors.map((prop, index) => {
				return <SnackbarContent message={t(prop)} color="danger" key={index} />;
			})}
		</GridItem>
	);
				...
				<RegularCard
					...
					content={
						...
						>
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="primary" onClick={decryptData} type="submit">
										{t("DECRYPT")}
									</Button>
						...
						>
							...
							<Grid container style={regular_login_style}>
								<GridItem xs={4} sm={4} md={4} style={{ marginTop: "20px" }}>
									...
									>
										<span style={!loginLoading ? {} : { display: "none" }}>
											{t("LOGIN")}
										</span>
										<BarLoader
											color={"#FFF"}
						...
						>
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									...
									/>
									<SnackbarContent
										message={t("IT_APPEARS_THAT_YOU_WANT_TO_CONNECT")}
										close
										color="info"
									/>
								</GridItem>
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="primary" onClick={approveHost} type="submit">
										{t("APPROVE")}
									</Button>
									<Button color="transparent" onClick={cancel}>
										{t("CANCEL")}
				...
				<RegularCard
					...
					content={
						...
						>
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="primary" onClick={cancel} type="submit">
										{t("BACK")}
									</Button>
								</GridItem>
							</Grid>
					...
					content={
						...
						>
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									<CustomInput
										labelText={t("FINGERPRINT_OF_THE_NEW_SERVER")}
										id="server_fingerprint"
										formControlProps={{
											fullWidth: true,
										}}
							...
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									<CustomInput
										labelText={t("FINGERPRINT_OF_THE_OLD_SERVER")}
										id="server_fingerprint"
										formControlProps={{
											fullWidth: true,
										}}
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									...
									<Button color="transparent" onClick={approveHost}>
					...
					content={
						...
						>
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									<SnackbarContent
										message={t("SOLVE_THE_WEBAUTHN_CHALLENGE_EXPLAINED")}
										close
										color="info"
									/>
								</GridItem>
				...
				<RegularCard
					...
					content={
						...
						>
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="transparent" onClick={cancel} fullWidth>
										{t("CANCEL")}
									</Button>
								</GridItem>
							</Grid>
						...
						>
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									...
									>
										<img src={iValtLogo} alt="ivalt" style={{ width: 50 }} />
										<CircularProgress
											size={60}
				...
				<RegularCard
					...
					content={
						...
						>
								...
								<GridItem xs={12} sm={12} md={12}>
									<CustomInput
										...
										inputProps={{
											...
											onChange: onChangeGoogleAuthentication,
										}}
									/>
								</GridItem>
							</Grid>
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="primary" onClick={verify_totp} type="submit">
										{t("SEND")}
									</Button>
									<Button color="transparent" onClick={cancel}>
										{t("CANCEL")}
				...
				<RegularCard
					...
					content={
						...
						>
							...
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									<Button color="primary" onClick={verify_duo} type="submit">
										{t("SEND")}
									</Button>
									<Button color="transparent" onClick={cancel}>
										{t("CANCEL")}
						...
						>
							<Grid container>
								<GridItem xs={12} sm={12} md={12}>
									<SnackbarContent
										color="warning"
										message={t("ACCEPTING_THIS_WILL_SEND_YOUR_PLAIN_PASSWORD")}
									/>
								</GridItem>
							</Grid>
							<Grid container>
								<GridItem xs={12} sm={4} md={12} style={{ marginTop: "20px" }}>
									...
									<Button color="success" onClick={disapproveSendPlain}>
										{t("DISAPPROVE_UNSAFE")}
									</Button>
								</GridItem>
							</Grid>
