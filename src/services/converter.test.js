...
describe('Service: converter test suite', function () {
    ...
    it('to_hex returns real hex values', () => {
        return expect(
            converter.to_hex(
                new Uint8Array([
    ...
    it('from_hex returns the true Uint8Array', () => {
        return expect(
            converter.to_hex(
                converter.from_hex('000102030405060708090a0b0c0d0e0f')
            )
        ).toBe('000102030405060708090a0b0c0d0e0f');
    });

    it('to_base58 returns the true Uint8Array', () => {
        return expect(
            converter.to_base58(
                new Uint8Array([
    ...
    it('from_base58 returns the true Uint8Array', () => {
        return expect(
            converter.to_base58(
                converter.from_base58(
                    '17zGKMk8LJ2vxPFJLY5ZT29kPLxuY4YedQ2wsCWP5aYENhQ93SGhYcc3XZaWR5w7pEvXozuf3daKVr'
                )
            )
        ).toBe(
    ...
    it('hex_to_base58', () => {
        return expect(
            converter.hex_to_base58('000102030405060708090a0b0c0d0e0f')
        ).toBe('12drXXUifSrRnXLGbXg8E');
    });

    it('base58_to_hex', () => {
        return expect(converter.base58_to_hex('12drXXUifSrRnXLGbXg8E')).toBe(
    ...
    it('uuid_to_hex', () => {
        return expect(
            converter.uuid_to_hex('3682454d-d080-44c2-b58c-721ef6459e32')
        ).toBe('3682454dd08044c2b58c721ef6459e32');
    });

    it('hex_to_uuid', () => {
        return expect(
            converter.hex_to_uuid('28b461d094d84a32b546f8cc382d49f0')
        ).toBe('28b461d0-94d8-4a32-b546-f8cc382d49f0');
    });

    it('words_to_hex', () => {
        return expect(
            converter.words_to_hex([
                'lazy',
                'lock',
                'lock',
                'price',
    ...
    it('hex_to_words', () => {
        return expect(
            converter.hex_to_words('000102030405060708090a0b0c0d0e0f')
        ).toEqual([
            'lazy',
            'lock',
            'lock',
    ...
    it('from_base_x:val.length = 0', () => {
        return expect(converter.from_base_x('', 'AB')).toEqual(
            new Uint8Array(0)
        );
    });
