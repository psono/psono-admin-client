...
function initiate_login(username, server, remember_me, trust_device) {
    ...
    return host.check_host(server).then(
        (response) => {
            return response;
        }
    );
...
function activate_token() {
    ...
    const onSuccess = function (activation_data) {
        ...
        action.set_user_info_3(activation_data.data.user.id, activation_data.data.user.email, user_secret_key);
    ...
    return psono_server.activate_token(token, verification.text, verification.nonce, session_secret_key)
        .then(onSuccess);
}
...
function handle_login_response(response, password, session_keys, server_public_key) {
    ...
    action.set_user_info_2(user_private_key, user_public_key, session_secret_key, token, user_sauce);
...
function login(password, server_info) {
    ...
    return psono_server.login(login_info_enc['text'], login_info_enc['nonce'], session_keys.public_key, session_duration)
