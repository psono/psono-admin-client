...
skopeo copy --all docker://psono-docker.jfrog.io/psono/psono-admin-client:latest docker://docker.io/psono/psono-admin-client:latest

export docker_version_tag=$(echo $CI_COMMIT_TAG | awk  '{ string=substr($0, 2, 100); print string; }' )
skopeo copy --all docker://psono-docker.jfrog.io/psono/psono-admin-client:latest docker://docker.io/psono/psono-admin-client:$docker_version_tag
